...
function viewSnippets() {
  ...
  if (snippetsIframe) {
    snippetsIframe.classList.add('rounded-md')
    ...
    style.appendChild(
      document.createTextNode(isDarkMode() ? darkStyles : styles)
    )
  }
}
/**
...
function builderUrl(glue) {
  // store.state.url no need to be prepended?
  return `${store.state.url}/assets/contentbuilder/${glue}`
}
