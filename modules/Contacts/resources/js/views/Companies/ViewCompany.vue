        ...
        <div class="bg-white px-3 py-4 dark:bg-neutral-900 sm:p-6">
          <div class="lg:flex lg:items-center lg:justify-between">
            <div class="lg:mr-5 lg:flex lg:items-center lg:space-x-5">
              <div v-once class="flex shrink-0 self-start lg:block">
                <CompanyImage class="mx-auto" />
              </div>
              ...
              >
                <template #label="{ label }">
                  <span
                    ...
                    v-i-tooltip.top="$t('contacts::fields.companies.user.name')"
                  >
                    <IAvatar
                      size="xs"
                      class="mr-1.5"
...
import Actions from '~/Core/resources/js/components/Actions'
...
import DetailsSection from '../../components/ViewCompanySidebar/ViewCompanyDetails.vue'
import DealsSection from '../../components/ViewCompanySidebar/ViewCompanyDeals.vue'
import ContactsSection from '../../components/ViewCompanySidebar/ViewCompanyContacts.vue'
import MediaSection from '../../components/ViewCompanySidebar/ViewCompanyMedia.vue'
import ChildCompaniesSection from '../../components/ViewCompanySidebar/ViewCompanyChildCompanies.vue'
...
const resourceName = Innoclapps.config('resources.companies.name')
...
const template = ref(Innoclapps.config('resources.companies.frontend.view'))
...
async function update(payload = {}) {
  let company = await updateForm
    .clear()
    .set(payload)
    .put(`/companies/${record.value.id}`)

  setRecord(company)
  updateFieldsValues(company)
...
watch(
  () => route.params,
  (newVal, oldVal) => {
    if (route.name === 'view-company' && newVal.id !== oldVal.id) {
      boot()
    }
  }
)
