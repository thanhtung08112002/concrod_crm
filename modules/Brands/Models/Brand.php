...
class Brand extends Model implements HasVisibilityGroups
{
    ...
    public static function getInitialAttributes(): array
    {
        return [
            ...
            'config.document.mail_message.en' => '',
    ...
    public function purge(): void
    {
        if ($this->documents()->count() > 0) {
            abort(409, __('brands::brand.delete_documents_usage_warning'));
        }

        // We must delete the trashed documents when the brand is deleted
        // as we don't have any option to do with the documents except to associate
        // it with other brand but won't be accurate.
        $this->documents()->onlyTrashed()->get()->each->forceDelete();
