...
class BelongsToColumn extends RelationshipColumn
{
    ...
    public function orderBy(Builder $query, array $order): Builder
    {
        ...
        $query->leftJoin(
            $table.' as '.$alias,
            fn ($join) => $join->on($instance->getForeignKeyName(), '=', $alias.'.id')
        );

        if (is_callable($this->orderByUsing)) {
            return call_user_func_array($this->orderByUsing, [$query, $order, $alias, $this]);
        }

        return $query->orderBy(
            $alias.'.'.$this->relationField, $order['direction']
        );
    }
}
