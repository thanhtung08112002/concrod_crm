...
class DocumentModelTest extends TestCase
{
    ...
    public function test_document_has_path_attribute()
    {
        $document = Document::factory()->create();

        $this->assertEquals('/documents/1', $document->path);
    }
    ...
    public function test_document_has_display_name_attribute()
    {
        ...
        $this->assertEquals('Document title', $document->display_name);
    }
    ...
    public function test_document_content_is_provided_as_pending_media_attribute()
    {
        ...
        $this->assertTrue(method_exists($document, 'attributesWithPendingMedia'));
        $this->assertEquals('content', $document->attributesWithPendingMedia());
    }
